...
export const appRouter = router({
  ...
  parcelas: router({
    ...
    // Parcelas pagas no mesmo mês da venda (entram no coletado normal do consultor)
    parcelasMesmoMesByConsultor: protectedProcedure
      .input(z.object({ consultorId: z.number(), mes: z.number(), ano: z.number() }))
      .query(async ({ input }) => {
        ...
        return db.select({
          ...
          dataPagamento: parcelasTable.dataPagamento,
          clienteNome: vendasTable.clienteNome,
          ...
          .where(and(
            eq(vendasTable.consultorId, input.consultorId),
            eq(parcelasTable.status, "pago"),
            eq(vendasTable.cancelada, false),
            sql`MONTH(${parcelasTable.dataPagamento}) = ${input.mes}`,
            sql`YEAR(${parcelasTable.dataPagamento}) = ${input.ano}`,
            sql`MONTH(${vendasTable.dataVenda}) = ${input.mes}`,
            sql`YEAR(${vendasTable.dataVenda}) = ${input.ano}`
          ));
      }),
  ...
  consultorDetalhe: router({
    coletado: protectedProcedure
      .input(z.object({ consultorId: z.number(), mes: z.number(), ano: z.number() }))
      .query(async ({ input }) => getDetalheColetadoConsultor(input.consultorId, input.mes, input.ano)),
    faturado: protectedProcedure
      .input(z.object({ consultorId: z.number(), mes: z.number(), ano: z.number() }))
      .query(async ({ input }) => getDetalheFaturadoConsultor(input.consultorId, input.mes, input.ano)),
    comissao: protectedProcedure
      .input(z.object({ consultorId: z.number(), mes: z.number(), ano: z.number() }))
      .query(async ({ input }) => getDetalheComissaoConsultor(input.consultorId, input.mes, input.ano)),
    aReceber: protectedProcedure
      .input(z.object({ consultorId: z.number() }))
      .query(async ({ input }) => getDetalheAReceberConsultor(input.consultorId)),
    coletadoParcelas: protectedProcedure
      .input(z.object({ consultorId: z.number(), mes: z.number(), ano: z.number() }))
      .query(async ({ input }) => getDetalheColetadoParcelasConsultor(input.consultorId, input.mes, input.ano)),
  }),
});
